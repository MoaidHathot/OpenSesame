...
def generate_wav(buffer, path):
    output = ''.join(buffer)
    sample_width = p.get_sample_size(FORMAT)
    print 'saving to file'
    # stream.write(output)
    save_to_file(RATE, sample_width, output, path)
...
if __name__ == "__main__":
    try:
        ...
        wavPath = sys.argv[2]
        print "Message to save: '%s'" % rawMessage
        print "Path to save to: '%s'" % wavPath
        
        message = '__' + rawMessage + '__'
        print "Generated message is '%s'" % message
        ...
        generate_wav(buffer, wavPath)
        
        print 'Wav file was generated and saved successfully.'
